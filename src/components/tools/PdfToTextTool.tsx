...
export function PdfToTextTool() {
  ...
  const handleFilesChange = async (newFiles: File[]) => {
    ...
    setProgress(0);
    try {
      const blob = await pdfToText(newFiles[0], setProgress);
      setResult(blob);
  ...
  if (stage === "processing") {
    return (
      <ProcessingView progress={progress} message="Extracting text..." />
    );
  }
          ...
          <div className="max-h-80 overflow-y-auto rounded-xl border border-border/50 bg-muted/30 p-5 font-mono text-sm leading-relaxed">
            ...
            {preview.length > 5000 && (
              <p className="mt-4 text-muted-foreground">
                ... {preview.length - 5000} more characters (download for full text)
              </p>
            )}
          </div>
          <button
